...
public class F1EmAddReportTC extends BaseClass {
	AdLogin1 alp1;
	AdLogin2 alp2;
	AdSideMenu asm;
	...
	int cd, cm, cy;
	Logger log = LogManager.getLogger(F1EmAddReportTC.class);
	PrintWriter pw, pw1;
	StringBuilder sb;
	...
	public void FillReport(String Scenario, String textarea, String Department, String dd, String mm, String toastmsg)
		...
		ExpAdVRUI.addAll(Arrays.asList("Employee Name: " + StaffName,Department,rpdt,"Unchecked" ));	
		...
		//	Reporter.log(ExpAdVRUI.get(i) + "<==>" + adViewReportHeader + "<==>" + adEditReportHeader, true);
			soft.assertEquals(ExpAdVRUI.get(i), saEmViewReportHeader, "HeaderData element at index" + i + "Dont Matched");
...
//		ExpEmHPDLst.addAll(Arrays.asList(rpdt, Department, getTimeDate()));
		...
		log.info("Collecting task From Staff View...");
		Thread.sleep(2000);
		LinkedHashMap<String, String> TVinEmViewReport2 = evr.getEmReportFormTaskAndValue(driver);
